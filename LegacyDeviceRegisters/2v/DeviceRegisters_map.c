...
uint16_t reg_address_from_id(reg_id_t id, uint8_t motor) {
    ...
    if (m->per_motor) {
        ...
        return m->base_addr + (uint16_t)motor * (uint16_t)m->words;
    }
    return m->base_addr;
}
...
reg_lut_t reg_id_from_address(uint16_t address) {
    ...
    if (address < REG_ADDR_MIN || address > REG_ADDR_MAX) return (reg_lut_t){REG_ID__NONE, 0};
...
bool reg_address_check_range(uint16_t address, uint16_t count){
    ...
    const uint32_t end = start + (uint32_t)count - 1u;
    if (start < (uint32_t)REG_ADDR_MIN || end > (uint32_t)REG_ADDR_MAX) return false;
    return true;
}
